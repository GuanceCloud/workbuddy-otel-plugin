...
test("shell installer reapplies the explicit gtrace preset to an existing config", async () => {
  ...
  await fs.access(path.join(profileDir, "plugins", "cache", "guance", "workbuddy-otel-plugin", "0.1.8", "hooks", "hooks.json"));
});
...
test("fallback installation writes managed hooks and plugin registry entries", async () => {
  ...
  const pluginRoot = path.join(tempDir, "plugins", "cache", "guance", "workbuddy-otel-plugin", "0.1.8");
  ...
  assert.equal(registry.plugins["workbuddy-otel-plugin@guance"].version, "0.1.8");
...
test("shell installer fallback writes settings hooks and plugin registry when CLI is unavailable", async () => {
  ...
  assert.equal(settings.enabledPlugins["workbuddy-otel-plugin@guance"], true);
  assert.match(settings.hooks.Stop[0].hooks[0].command, /plugins\/cache\/guance\/workbuddy-otel-plugin\/0\.1\.8/);
